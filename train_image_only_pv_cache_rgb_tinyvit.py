...
# Se deja cuDNN desactivado para mantener el comportamiento del script base.
...
MODEL_OUT = "best_image_only_tinyvit_cache_rgb_no_autoreg.pt"
PREDICTIONS_OUT = "eval_predictions_image_only_tinyvit_cache_rgb_no_autoreg.tsv"
PLOT_OUT = "eval_timeline_image_only_tinyvit_cache_rgb_no_autoreg.png"
...
# ViT muy ligero. Trata T imagenes RGB como T*3 canales de un solo bloque.
IMG_EMB_DIM = 64
VIT_PATCH_SIZE = 8
VIT_DEPTH = 4
VIT_HEADS = 4
...
CV_SUMMARY_OUT = "cv5_temporal_scene_metrics_image_only_tinyvit_cache_rgb_no_autoreg.tsv"
CV_ALL_PREDICTIONS_OUT = "cv5_temporal_all_predictions_image_only_tinyvit_cache_rgb_no_autoreg.tsv"
CV_PRODUCTION_SCENE_METRICS_OUT = "cv5_production_scene_metrics_image_only_tinyvit_cache_rgb_no_autoreg.tsv"
...
# ============================================================
# Modelo SOLO imagen: Tiny ViT sin temporal explicito
# ============================================================

class TinySequenceViTEncoder(nn.Module):
    def __init__(
        self,
        sequence_len,
        img_emb_dim=64,
        ...
        self.patch_embed = nn.Conv2d(
            in_channels=sequence_len * EXPECTED_IMAGE_CHANNELS,
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(1, self.num_patches, img_emb_dim)
        )
    ...
    def forward(self, imgs):
        # imgs: [B, T, 3, H, W]. Se apila tiempo+RGB como canales: [B, T*3, H, W].
        b, t, c, h, w = imgs.shape

        if c != EXPECTED_IMAGE_CHANNELS:
            raise RuntimeError(
                f"Este TinyViT espera imagenes de {EXPECTED_IMAGE_CHANNELS} canales, recibio C={c}."
            )
        if t != self.sequence_len:
            raise RuntimeError(
                f"Este TinyViT fue creado para T={self.sequence_len}, recibio T={t}."
            )
...
class ImageOnlyPVModel(nn.Module):
    def __init__(self, img_emb_dim=64):
        super().__init__()

        self.img_encoder = TinySequenceViTEncoder(
            sequence_len=W,
            ...
            torch.save(
                {
                    ...
                    "model_type": "image_only_tinyvit_cache_rgb_no_autoreg_cv5_temporal",
                    "fold_id": fold_id,
                    "n_folds": N_FOLDS,
                    ...
                    "img_emb_dim": IMG_EMB_DIM,
                    "sequence_as_channels": True,
                    "rgb_sequence_as_channels": True,
                    "sequence_len": W,
...
def main():
    ...
    print("Modo: SOLO imágenes cacheadas RGB + TinyViT por bloque [B,T*3,H,W]")
