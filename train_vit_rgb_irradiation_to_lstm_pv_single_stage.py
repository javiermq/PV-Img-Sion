...
IMAGE_CACHE_PATH = Path("data/images_cache_rgb.pt")
IMG_SIZE = 64
...
TARGET_IRRADIATION_COLUMN = "irradiation"
TARGET_PRODUCTION_COLUMN = "production"
IRRADIATION_LOSS_WEIGHT = 0.35
...
# Normalización de salida: y_norm = production / capacity.
# No hay MinMax de X porque este script NO usa weather ni tabular.
EPS = 1e-8
...
MODEL_OUT = "best_vit_rgb_irradiation_to_lstm_pv_single_stage.pt"
PREDICTIONS_OUT = "eval_predictions_vit_rgb_irradiation_to_lstm_pv_single_stage.tsv"
PLOT_OUT = "eval_timeline_vit_rgb_irradiation_to_lstm_pv_single_stage.png"

CV_SUMMARY_OUT = "cv5_temporal_scene_metrics_vit_rgb_irradiation_to_lstm_pv_single_stage.tsv"
CV_ALL_PREDICTIONS_OUT = "cv5_temporal_all_predictions_vit_rgb_irradiation_to_lstm_pv_single_stage.tsv"
CV_PRODUCTION_SCENE_METRICS_OUT = "cv5_production_scene_metrics_vit_rgb_irradiation_to_lstm_pv_single_stage.tsv"
...
def load_image_cache(cache_path):
    ...
    if not cache_path.exists():
        raise RuntimeError(
            ...
            "--out data/images_cache_rgb.pt --root . --image-size 64 --mode rgb"
        )
...
# ============================================================
# Construcción rápida de samples SOLO imagen desde cache
# ============================================================

def build_image_only_samples_from_cache(
    df,
    window,
    ...
    Para cada label_idx:
      ...
      - X_img = W imágenes anteriores existentes EN CACHE.

    IMPORTANTE:
      - NO usa weather.
      - NO usa production como input.
      - NO usa imagen del mismo timestamp objetivo.
      - Las imágenes NO se leen de disco; se exige que image_path esté en path_to_idx.
    """
    print()
    print("Construyendo samples SOLO imagen rápidos desde cache...", flush=True)
    ...
    y_values = df[TARGET_PRODUCTION_COLUMN].values.astype(np.float32)
...
# ============================================================
# Dataset SOLO imagen usando imágenes cacheadas
# ============================================================

class CachedImageOnlyPVForecastDataset(Dataset):
    def __init__(
        self,
        df,
        samples,
        ...
        self.irr_range = irr_range
        self.prod_min = prod_min
        self.prod_range = prod_range
        self.cache_images_uint8 = cache_images_uint8

        irr_raw = self.df[TARGET_IRRADIATION_COLUMN].values.astype(np.float32)
        prod_raw = self.df[TARGET_PRODUCTION_COLUMN].values.astype(np.float32)

        self.irr = minmax_y(irr_raw, self.irr_min, self.irr_range).astype(np.float32)
        self.prod = minmax_y(prod_raw, self.prod_min, self.prod_range).astype(np.float32)
    ...
    def __getitem__(self, idx):
        ...
        irr_window = self.irr[image_indices]
        prod = self.prod[label_idx]
        ...
        return (
            imgs,
            torch.tensor(irr_window, dtype=torch.float32),
            torch.tensor(prod, dtype=torch.float32),
        )


# ============================================================
# Modelo imagen -> irradiance -> LSTM PV
# ============================================================
...
class TinyViTFrameEncoder(nn.Module):
    def __init__(
        ...
        if img_size % patch_size != 0:
            raise ValueError("img_size debe ser divisible por patch_size.")
...
class ViTImageSequenceEncoder(nn.Module):
    def __init__(
        self,
        frame_emb_dim=128,
        lstm_hidden=128,
        ...
        self.frame_encoder = TinyViTFrameEncoder(
            img_size=IMG_SIZE,
            patch_size=8,
            in_channels=EXPECTED_IMAGE_CHANNELS,
            emb_dim=frame_emb_dim,
            depth=3,
            num_heads=4,
            dropout=0.10,
        )
...
class ImageIrradiationToPVSingleStageModel(nn.Module):
    def __init__(self):
        super().__init__()

        self.img_encoder = ViTImageSequenceEncoder(
            frame_emb_dim=128,
            lstm_hidden=128,
            lstm_layers=2,
            lstm_dropout=0.2,
        ...
        self.irradiation_head = nn.Sequential(
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Dropout(0.10),
            nn.Linear(64, 1),
        ...
        self.production_regressor = nn.Sequential(
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(64, 1),
    ...
    def forward(self, imgs):
        img_seq = self.img_encoder(imgs)
        irr_seq = self.irradiation_head(img_seq).squeeze(-1)
        pv_input = irr_seq.unsqueeze(-1)
        pv_out, _ = self.production_lstm(pv_input)
        prod_hat = self.production_regressor(pv_out[:, -1, :]).squeeze(1)
        return irr_seq, prod_hat
...
def train_one_epoch(model, loader, optimizer, criterion):
    ...
    for batch_idx, (imgs, irr, prod) in enumerate(loader):
        if batch_idx % 20 == 0:
            print(f"  batch {batch_idx}/{len(loader)}", flush=True)

        imgs = imgs.to(DEVICE, non_blocking=True)
        irr = irr.to(DEVICE, non_blocking=True)
        prod = prod.to(DEVICE, non_blocking=True)

        optimizer.zero_grad()

        irr_pred, prod_pred = model(imgs)
        loss_irr = criterion(irr_pred, irr)
        loss_prod = criterion(prod_pred, prod)
        loss = loss_prod + IRRADIATION_LOSS_WEIGHT * loss_irr
...
def evaluate(
    ...
    for batch_idx, (imgs, irr, prod) in enumerate(loader):
        ...
        prod_pred_real = inverse_minmax_y(
            prod_pred.detach().cpu().numpy(),
            prod_min,
            prod_range,
        )

        prod_real = inverse_minmax_y(
            prod.detach().cpu().numpy(),
            prod_min,
            prod_range,
        )

        irr_pred_real = inverse_minmax_y(
            irr_pred.detach().cpu().numpy(),
            irr_min,
            irr_range,
        )

        irr_real = inverse_minmax_y(
            irr.detach().cpu().numpy(),
            irr_min,
            irr_range,
        )

        prod_preds_all.append(prod_pred_real)
        prod_y_all.append(prod_real)
        irr_preds_all.append(irr_pred_real)
        irr_y_all.append(irr_real)

    prod_preds_all = np.concatenate(prod_preds_all)
    prod_y_all = np.concatenate(prod_y_all)
    irr_preds_all = np.concatenate(irr_preds_all, axis=0)
    irr_y_all = np.concatenate(irr_y_all, axis=0)

    arr_metrics = compute_metrics_from_arrays(
        y_true=prod_y_all,
        y_pred=prod_preds_all,
        capacity=capacity,
    )
    irr_metrics = compute_metrics_from_arrays(
        y_true=irr_y_all.reshape(-1),
        y_pred=irr_preds_all.reshape(-1),
        capacity=irr_range,
    )

    metrics = {
        "loss": total_loss / len(loader.dataset),
        "irr_mae": irr_metrics["mae"],
        "irr_rmse": irr_metrics["rmse"],
        "irr_mae_rel_pct": irr_metrics["mae_pct_mean"],
        "irr_rmse_rel_pct": irr_metrics["rmse_pct_mean"],
        "mae": arr_metrics["mae"],
    ...
    if return_arrays:
        metrics["preds"] = prod_preds_all
        metrics["y_true"] = prod_y_all
        metrics["irr_preds"] = irr_preds_all
        metrics["irr_y_true"] = irr_y_all

    return metrics


@torch.no_grad()
def evaluate_mean_train_baseline(loader, prod_min, prod_range, train_y_mean_real, capacity):
    preds_all = []
    y_all = []

    for imgs, irr, prod in loader:
...
def run_one_temporal_fold(
    ...
    train_irr_indices = sorted(
        {
            idx
            for sample in train_samples
            for idx in sample["image_indices"]
        }
    )

    train_scaler_df = df.iloc[train_label_indices].reset_index(drop=True)
    train_irr_values = df.loc[train_irr_indices, TARGET_IRRADIATION_COLUMN].values.astype(np.float32)

    irr_min = float(train_irr_values.min())
    irr_max = float(train_irr_values.max())
    ...
    prod_min = 0.0
    prod_range = float(capacity)

    train_dataset = CachedImageOnlyPVForecastDataset(
        df=df,
    ...
    eval_dataset = CachedImageOnlyPVForecastDataset(
        df=df,
    ...
    print(f"  irradiation_min train: {irr_min:.4f}")
    print(f"  irradiation_range train: {irr_range:.4f}")
    print(f"  irradiation_max train: {irr_min + irr_range:.4f}")
    print(f"  production_min train: {prod_min:.4f}")
    print(f"  production_range train: {prod_range:.4f}")
    print(f"  production_max train: {prod_min + prod_range:.4f}")
    print("  Entradas weather/tabular: NO")
    print("  Entrada production/autoregresiva: NO")
    print("  Entrada imágenes: SÍ")
    ...
    train_y_mean_real = float(train_scaler_df[TARGET_PRODUCTION_COLUMN].mean())
    ...
    model = ImageIrradiationToPVSingleStageModel().to(DEVICE)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        eval_irr_mae = metrics["irr_mae"]
        eval_irr_rmse = metrics["irr_rmse"]
        ...
        print(
            ...
            f"eval_irr_MAE={eval_irr_mae:.2f} | "
            f"eval_irr_RMSE={eval_irr_rmse:.2f} | "
        ...
        if improved:
            ...
            best_metrics_to_save = {
                ...
                if k not in ["preds", "y_true", "irr_preds", "irr_y_true"]
            }
            ...
            torch.save(
                {
                    ...
                    "input_cols": [],
                    "target_irradiation_column": TARGET_IRRADIATION_COLUMN,
                    "target_production_column": TARGET_PRODUCTION_COLUMN,
                    "irr_min": irr_min,
                    "irr_range": irr_range,
                    "prod_min": prod_min,
                    "prod_range": prod_range,
                    "irradiation_loss_weight": IRRADIATION_LOSS_WEIGHT,
                    "window": W,
                    "img_size": IMG_SIZE,
                    "image_cache_path": str(IMAGE_CACHE_PATH),
                    "image_cache_mode": EXPECTED_CACHE_MODE,
                    "image_channels": EXPECTED_IMAGE_CHANNELS,
                    "model_type": "vit_rgb_irradiation_to_lstm_pv_single_stage_cv5_temporal",
                    "fold_id": fold_id,
                    "n_folds": N_FOLDS,
                    "uses_weather_as_input": False,
                    "uses_tabular_as_input": False,
                    "uses_predicted_irradiation_as_pv_input": True,
                    "uses_production_as_input": False,
                    "uses_images_as_input": True,
                    "uses_cached_images": True,
                    "image_encoder": "tiny_vit_rgb_patch8_emb128_depth3_heads4",
                    "image_sequence_hidden": 128,
                    "img_lstm_hidden": 128,
                    "img_lstm_layers": 2,
    ...
    pred_df = pd.DataFrame(
        {
            ...
            "irradiation_true_last_image": final_metrics["irr_y_true"][:, -1],
            "irradiation_pred_last_image": final_metrics["irr_preds"][:, -1],
            "irradiation_abs_error_last_image": np.abs(
                final_metrics["irr_preds"][:, -1]
                - final_metrics["irr_y_true"][:, -1]
            ),
    ...
    result = {
        ...
        "best_eval_irr_mae": best_metrics["irr_mae"],
        "best_eval_irr_rmse": best_metrics["irr_rmse"],
...
def main():
    ...
    print("Modo: ViT RGB -> irradiation auxiliar -> LSTM predicted-irradiation -> production")
    ...
    print(f"Target auxiliar: {TARGET_IRRADIATION_COLUMN}")
    print(f"Target final: {TARGET_PRODUCTION_COLUMN}")
    ...
    for col in [TARGET_IRRADIATION_COLUMN, TARGET_PRODUCTION_COLUMN]:
        if col not in df.columns:
            raise RuntimeError(f"No existe la columna obligatoria '{col}'.")

    before = len(df)
    for col in [TARGET_IRRADIATION_COLUMN, TARGET_PRODUCTION_COLUMN]:
        df[col] = pd.to_numeric(df[col], errors="coerce")
    df = df.dropna(subset=[TARGET_IRRADIATION_COLUMN, TARGET_PRODUCTION_COLUMN]).reset_index(drop=True)
    after = len(df)

    global CAPACITY
    CAPACITY = float(
        np.percentile(
            df[TARGET_PRODUCTION_COLUMN].dropna().values.astype(np.float32),
            CAPACITY_PERCENTILE,
        )
    )

    print()
    print(f"CAPACITY p{CAPACITY_PERCENTILE:.1f} {TARGET_PRODUCTION_COLUMN}: {CAPACITY:.2f}")
    ...
    print(f"  - auxiliar: {TARGET_IRRADIATION_COLUMN}")
    print(f"  - final: {TARGET_PRODUCTION_COLUMN}")
    ...
    print(f"Filas tras dropna targets: {after}")
    ...
    all_samples, has_cached_image = build_image_only_samples_from_cache(
        df=df,
        window=W,
        max_image_age_minutes=MAX_IMAGE_AGE_MINUTES,
    ...
    if len(all_samples) == 0:
        raise RuntimeError(
            "No hay samples de imagen válidos. "
    ...
    print(f"Samples SOLO imagen válidos totales: {len(all_samples)}")
    ...
    print(f"  Label {TARGET_IRRADIATION_COLUMN}: {df.loc[first['label_idx'], TARGET_IRRADIATION_COLUMN]:.2f}")
    print(f"  Label {TARGET_PRODUCTION_COLUMN}: {df.loc[first['label_idx'], TARGET_PRODUCTION_COLUMN]:.2f}")
    print("  Imágenes anteriores cacheadas:")
    ...
    print()
    print("Cross-validation temporal SOLO imagen terminada.")
